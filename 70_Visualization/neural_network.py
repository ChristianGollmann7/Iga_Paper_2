...
class Mlp(nn.Module):
    def __init__(self,
        ...
        :param hidden: size of each hidden layer whereas the input size gets multiplied with respective factor, must contain at least one entry
        ...
        network_layers.append(nn.Linear(number_inputs, int(np.maximum(number_inputs*hidden[0], 1))))
        network_layers.append(activation())

        # Connect the rest of the hidden layers
        for i in range(len(hidden)-1):
            network_layers.append(nn.Linear(int(np.maximum(number_inputs*hidden[i], 1)), int(np.maximum(number_inputs*hidden[i+1], 1))))
            network_layers.append(activation())

        # Connect the output layer, no activation function
        network_layers.append(nn.Linear(int(np.maximum(number_inputs*hidden[-1], 1)), number_outputs))
