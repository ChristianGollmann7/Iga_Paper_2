...
"""
Results 2
This file postprocesses the results of the vanilla analysis. 
Creates various graphics and saves them to "vanilla_results" folder.
"""
# Enter here the same sample numbers an architecture as in the vanilla_analysis
number_training_samples = [1, 10, 100, 1000]
architecture = [1, 1]

c_gismo = "seagreen"
c_network = "darkorange"

# Load the results produced by vanilla_analysis.py
with open(f"results_2/metrics_{architecture}.yaml") as f:
...
fig.savefig(f"results_2/loss_plot_{architecture}.svg",
            format="svg",
            bbox_inches='tight',
            pad_inches=0.05)
...
# Time and error plot
pos = np.arange(len(number_training_samples))
labels = [str(i) for i in number_training_samples]
bar_width = 0.2

fig, ax = plt.subplots(figsize=(12, 8))

bars_1 = ax.bar(pos-bar_width/2*3, prep_time, bar_width, label='preparation time', color='tab:blue')
bars_2 = ax.bar(pos-bar_width/2, run_time, bar_width, label='training time', color='lightblue')
ax.set_xticks(pos)
...
ax2 = ax.twinx()
bars_3 = ax2.bar(pos+bar_width/2, error, bar_width, label='average validation error', color='orange')
ax2.set_ylabel('validation error [length units]', fontsize=14, color='orange')
ax2.tick_params(axis='y', colors='orange')
ax2.set_yscale('log')

handles_1, labels_1 = ax.get_legend_handles_labels()
handles_2, labels_2 = ax2.get_legend_handles_labels()
ax.legend(handles_1+handles_2, labels_1+labels_2, fontsize=14, loc='upper center')
...
ax.set_title("Preparation / Training time and validation error for different numbers of training samples", fontsize=16)

plt.tight_layout(rect=[0, 0, 1, 0.99])
fig.savefig(f"results_2/error_plot_{architecture}.svg",
            format="svg",
            bbox_inches='tight',
            pad_inches=0.05)
...
def create_visu_plot(delta_x, delta_y, poisson, net_architecture, number_training_samples):
    ...
    for i in range(5):
        # Load the model
        model = torch.load(f"results_2/network_{net_architecture}_{number_training_samples}.pth", weights_only=False)
        ...
        ax[i].set_facecolor((1.0, 1.0, 1.0))
    ...
    plt.tight_layout(rect=[0, 0, 1, 0.99])
    fig.savefig(f"results_2/displacement_plot_{net_architecture}_{number_training_samples}.svg",
                format="svg",
                bbox_inches='tight',
                pad_inches=0.05)
