...
Creates various graphics and saves them to "multi_instance_results_presampled" folder.
"""
...
compare_model_results = f"results_3_paper/physics/metrics_r_8.yaml"
...
networks = [f"results_3/physics/network_{architecture}_r_8_{i}.pth" for i in number_training_samples]
for network in networks:
    model = torch.load(network, weights_only=False)
    result_pinn = Pinn(model)
    val_error_max, val_error_mean = evaluate_validation_samples(result_pinn, dirichlet_boundaries, neumann_boundaries, validation_set, return_value='both')
...
square_handle = mpatches.Patch(facecolor='none', edgecolor='black', linewidth=2, label='trained with simple BCs')
#ax.legend(handles_1+handles_2+[square_handle], labels_1+labels_2+['trained with simple BCs'], fontsize=14, loc="upper left", bbox_to_anchor=(0.45, 1.0), borderaxespad=0, frameon=True, framealpha=1.0)
ax.legend(
    handles_1 + handles_2 + [square_handle],
    labels_1 + labels_2 + ['trained with simple BCs'],
    fontsize=14,
    loc="upper center",
    bbox_to_anchor=(0.5, -0.12),
    ncol=len(labels_1 + labels_2 + ['trained with simple BCs']),
    borderaxespad=0,
    frameon=False,
    framealpha=1.0)
...
#ax.set_title("Comparison between physics-based network trained on complete and simple BCs", fontsize=16)



plt.tight_layout(rect=[0, 0, 1, 0.99])
fig.savefig(f"results_3_paper/physics/error_plot_combined_simple_vs_hard_{architecture}.pdf",
            format="pdf",
            bbox_inches='tight',
            pad_inches=0.05)
...
number_training_samples = [1, 10, 100, 1000]
architecture = [1, 1]
base_model_results = f"results_2_paper/metrics_{architecture}.yaml"
compare_model_results = f"results_3_paper/supervised/metrics.yaml"
...
networks = [f"results_3_paper/supervised/network_{architecture}_{i}.pth" for i in number_training_samples]
for network in networks:
    model = torch.load(network, weights_only=False)
    result_pinn = Pinn(model)
    val_error_max, val_error_mean = evaluate_validation_samples(result_pinn, dirichlet_boundaries, neumann_boundaries, validation_set, return_value='both')
...
#ax.set_title("Comparison between supervised network trained on complete and simple BCs", fontsize=16)



plt.tight_layout(rect=[0, 0, 1, 0.99])
fig.savefig(f"results_3_paper/supervised/error_plot_combined_simple_vs_hard_{architecture}.pdf",
            format="pdf",
            bbox_inches='tight',
            pad_inches=0.05)
