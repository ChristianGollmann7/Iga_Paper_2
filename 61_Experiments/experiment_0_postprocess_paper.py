...
refinements = 7
architecture = [0]
use_gauss = True        # switch here for gauss or no gauss
...
for i in range(refinements+1)[1:]:
    ...
    error, poly_data = get_difference_visu_data(G, S_gismo, S_net, resolution=32, relative=False, plot_reference='S_net')
    ...
    if not use_gauss:
        ...
        fig.savefig(f"results_0_paper/no_gauss/error_plot_{i}.pdf", dpi=600, bbox_inches='tight', pad_inches=0.05)
    else:
        ax.set_title(f"absolute error\n{i} Gauss points", fontsize=24)
        fig.savefig(f"results_0_paper/gauss/error_plot_{i}.pdf", dpi=600, bbox_inches='tight', pad_inches=0.05)
...
ax.legend()
ax.set_ylabel("Wall-Clock Time (in s)", fontsize=14)
...
if not use_gauss:
    ...
    fig.savefig(f"results_0_paper/no_gauss/runtime_plot.pdf", dpi=600, bbox_inches='tight', pad_inches=0.05)
else:
    ...
    fig.savefig(f"results_0_paper/gauss/runtime_plot.pdf", dpi=600, bbox_inches='tight', pad_inches=0.05)
...
ax2 = ax.twinx()
l2 = ax2.plot(x, error_list, color="red", marker="^", label="Cartesian Error")
ax2.tick_params(axis="y", colors="red")
ax2.set_ylabel("max Cartesian Error", fontsize=14, color="red")
ax2.set_yscale('log')
ax.tick_params(axis='both', which='major', labelsize=14, length=6, width=1.5)
...
if not use_gauss:
    ...
    fig.savefig(f"results_0_paper/no_gauss/error_plot.pdf", dpi=600, bbox_inches='tight', pad_inches=0.05)
else:
    ax.set_xlabel("one directional Gauss points per knot span", fontsize=14)
    ...
    fig.savefig(f"results_0_paper/gauss/error_plot.pdf", dpi=600, bbox_inches='tight', pad_inches=0.05)
